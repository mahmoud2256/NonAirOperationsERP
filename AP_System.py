...
def open_system():
    ...
    def update_handling_fees(event=None):

        try:

            paid = float(paid_to_supplier_entry.get() or 0)

            selected_account = accounts_combo.get()

            rate = get_handling_rate(selected_account)
            ...
            if rate is not None:
                ...
                handling_fees_entry.insert(0, f"{handling:.2f}")
    ...
    paid_to_supplier_entry.bind("<KeyRelease>", update_handling_fees)
